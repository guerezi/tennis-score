...
export const cloneState = (state: MatchState): MatchState =>
  JSON.parse(JSON.stringify(state));

const getOtherPlayer = (id: PlayerId): PlayerId =>
...
export const addPoint = (
    ...
    if (!wasTieBreak) {
      ...
      if (state.config.mode === "doubles") {
        if (state.server === PlayerId.P1)
          state.p1ServerIdx = 1 - (state.p1ServerIdx || 0);
  ...
  if (setWon) {
    ...
    if (
      p1Wins === state.config.setsToWin ||
      p2Wins === state.config.setsToWin
    ) {
      matchWon = true;
      eventType = "MATCH_WIN";
      ...
      if (
        state.config.finalSetType === "superTieBreak" &&
        state.currentSetIndex === state.config.setsToWin * 2 - 2
      ) {
        state.isTieBreak = true;
        state.points = { [PlayerId.P1]: 0, [PlayerId.P2]: 0 };
  ...
  if (state.isTieBreak) {
    const pts =
      (state.points[PlayerId.P1] as number) +
      (state.points[PlayerId.P2] as number);
