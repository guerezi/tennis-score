    ...
    <div className="flex-1 flex flex-col relative">
      ...
      <div className="flex justify-between items-center p-3 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1 gap-1">
          {onBack && (
            <button
              onClick={onBack}
              className="p-1.5 rounded-md hover:bg-white dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 shadow-sm transition-all"
            >
              <ArrowLeft size={18} />
            </button>
          )}
        ...
        <div className="flex justify-center items-center gap-6 relative z-10">
          ...
          <div className="flex flex-col items-center px-4 py-1 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div className="text-3xl font-mono font-black tracking-widest leading-none flex gap-3 text-slate-900 dark:text-white">
              <span className={p1Styles.primary}>
                {state.games[PlayerId.P1]}
              </span>
              <span className="text-slate-300 dark:text-slate-600">-</span>
              <span className={p2Styles.primary}>
      ...
      <div className="flex-1 flex flex-col justify-center gap-2 p-2 relative">
          ...
          <div className="flex items-center justify-between h-full px-6">
            <div className="text-left z-10 flex flex-col items-start gap-1">
              ...
              <div
                ...
                className={`text-2xl font-bold ${p1Styles.text} ${
                  p1Styles.darkText
                } truncate max-w-[150px] flex items-center gap-2 ${
                  isDoubles && state.server === PlayerId.P1
              ...
              {isDoubles && (
                <div
                  ...
                  className={`text-xl font-bold text-slate-600 dark:text-white/60 truncate max-w-[150px] flex items-center gap-2 ${
                    state.server === PlayerId.P1
                      ? "cursor-pointer hover:opacity-80"
                      : ""
                  }`}
              ...
              {state.server === PlayerId.P1 && !state.isMatchOver && (
                <div
                  className={`mt-1 ${p1Styles.badgeBg} ${p1Styles.badgeText} text-xs px-3 py-1 rounded-full inline-flex items-center gap-1 font-bold shadow-sm border border-current/20`}
                >
                  <span>SERVING</span>
                </div>
              )}
            ...
            <div className="z-10 text-right">
              <div
                className={`text-7xl font-mono font-black ${p1Styles.scoreColor} ${p1Styles.darkScoreColor} tracking-tighter shadow-blue-500 drop-shadow-lg`}
              >
                {p1Point}
              </div>
            </div>
        ...
        <div className="h-4 flex items-center justify-center">
          {state.isTieBreak ? (
            <span className="text-[10px] bg-purple-600 text-white px-3 py-0.5 rounded-full font-bold tracking-wider animate-pulse shadow-lg">
              TIE BREAK
            </span>
          ) : (
          ...
          <div className="flex items-center justify-between h-full px-6">
            <div className="text-left z-10 flex flex-col items-start gap-1">
              ...
              <div
                ...
                className={`text-2xl font-bold ${p2Styles.text} ${
                  p2Styles.darkText
                } truncate max-w-[150px] flex items-center gap-2 ${
                  isDoubles && state.server === PlayerId.P2
                    ? "cursor-pointer hover:opacity-80"
                    : ""
                }`}
              >
              ...
              {state.server === PlayerId.P2 && !state.isMatchOver && (
                <div
                  className={`mt-1 ${p2Styles.badgeBg} ${p2Styles.badgeText} text-xs px-3 py-1 rounded-full inline-flex items-center gap-1 font-bold shadow-sm border border-current/20`}
                >
                  <span>SERVING</span>
                </div>
              )}
            ...
            <div className="z-10 text-right">
              <div
                className={`text-7xl font-mono font-black ${p2Styles.scoreColor} ${p2Styles.darkScoreColor} tracking-tighter drop-shadow-lg`}
              >
                {p2Point}
              </div>
            </div>
      ...
      {state.isMatchOver && (
        <div className="absolute inset-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in duration-500">
            ...
            <span
              className={
                state.winner === PlayerId.P1
                  ? `${p1Styles.primary} font-bold`
                  : `${p2Styles.primary} font-bold`
              }
            >
          ...
          <div className="flex gap-4">
            <button
              onClick={onUndo}
              className="px-6 py-2 rounded-full border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              Undo Last Point
            </button>
            <button
              onClick={onReset}
              className="px-6 py-2 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold hover:opacity-90 transition-opacity"
            >
              Start New Match
            </button>
          </div>
