...
const SettingsModal: React.FC<Props> = ({ isOpen, onClose, config, onSave, onReset }) => {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded p-1">
                 <button 
                   ...
                   className={`px-3 py-1 rounded text-sm transition-colors ${formData.useAdvantage ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'}`}
                 >
                   Advantage
                 </button>
                 <button 
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded p-1">
                 <button 
                   ...
                   className={`px-3 py-1 rounded text-sm transition-colors ${formData.finalSetType === 'standard' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'}`}
                 >
                   Standard
                 </button>
                 <button 
                   onClick={() => handleChange('finalSetType', 'superTieBreak')}
                   className={`px-3 py-1 rounded text-sm transition-colors ${formData.finalSetType === 'superTieBreak' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'}`}
                 >
                   Super TB
                 </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
                <div>
                    ...
                    <input 
                        ...
                        onChange={(e) => handleChange('tieBreakAt', parseInt(e.target.value) || 6)}
                ...
                <div>
                    ...
                    <input 
                        ...
                        onChange={(e) => handleChange('tieBreakPoints', parseInt(e.target.value) || 7)}
                        className="w-full bg-slate-100 dark:bg-slate-900 rounded p-2 text-center text-sm font-bold dark:text-white border border-slate-200 dark:border-slate-700"
                    />
                </div>
            </div>
